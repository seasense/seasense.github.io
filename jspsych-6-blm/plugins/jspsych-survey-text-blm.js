...
jsPsych.plugins['survey-text-blm'] = (function() {
  ...
  plugin.info = {
    ...
    parameters: {
      ...
      minLength: {
        type: jsPsych.plugins.parameterType.INT,
            pretty_name: 'minLength',
            default: 1,
            description: 'The minimum number of characters to be entered.'
      },
      maxLength: {
        type: jsPsych.plugins.parameterType.INT,
            pretty_name: 'maxLength',
            default: 300,
            description: 'The maximum number of characters to be entered.'
      }
    }
  }
  ...
  plugin.trial = function(display_element, trial) {
    ...
    display_element.querySelector('#jspsych-survey-text-form').addEventListener('submit', function(e) {
      ...
      // save data
      var trialdata = {
        stimulus: trial.questions[0].prompt,
        rt: response_time,
        response: question_data.Q0,
        response_length: question_data.Q0.length
      };
      if (trialdata.response_length<trial.minLength) {alert("Please enter a text that is at least "+trial.minLength+" characters long.")}else{
        if (trialdata.response_length>trial.maxLength) {alert("Please enter a text that does not contain more than "+trial.maxLength+" characters.")}else{
