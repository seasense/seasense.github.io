...
jsPsych.plugins["selectExemplar"] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var i = 0; i < trial.choices.length; i++) {
      ...
      // Color of feedback on species-specific learning progress
      if(clicks_total<3){feedback_color="white"}else{
      	if(accuracy_i>=.75){feedback_color="green"}else{
      		if(accuracy_i<.75 && accuracy_i>=.50){feedback_color="yellow"}else{
      			if(accuracy_i<.50 && accuracy_i>=.25){feedback_color="orange"}else{
      				feedback_color="red"
      			}
      		}
      	}
